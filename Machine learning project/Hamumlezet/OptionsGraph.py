...
from settings import SEASON, MIN_POINTS_PER_SEMESTER, MAX_POINTS_PER_SEMESTER, DAYS, DAYS_BETWEEN_TESTS, COURSES_TYPES, \
    MAX_AVAILABLE_COURSES, LENGTH_OPTIMIZATION, MANDATORY_COURSES_POINTS
...
class OptionsGraph(Singleton):
    ...
    def __init__(self):
        ...
        self.__semesters = {MIN_POINTS_PER_SEMESTER: set()}
        ...
        self.options_coounter ={}

    @staticmethod
    def get_instance():
        if OptionsGraph._instance is None:
            raise Exception("Options Graph instance does not exist")
        return OptionsGraph._instance
    ...
    def get_next_season(self, current: str):
        if current == SEASON[0]:
            return SEASON[1]
        elif current == SEASON[1]:
            return SEASON[0]
        # elif current == SEASON[2]:
        #    return SEASON[0]

        return ""

    def __gen(self, index: int, comb_size: float, comb: list[str], state: State) -> None:
        if comb_size > MAX_POINTS_PER_SEMESTER:
            return

        semester = deepcopy(comb)
        key = comb_size
        # if comb_size >= MIN_POINTS_PER_SEMESTER:
        #     key = MIN_POINTS_PER_SEMESTER
    ...
    def get_the_most_relevant_courses(self, courses: {}, state: State) -> [str]:
        ...
        total_points = 0
        for typ in courses:
            if typ != COURSES_TYPES[8] and typ != COURSES_TYPES[9]:
                left_points = state.degree_requirements[typ][1]
                if left_points > 0:
                    left_points_per_type[typ] = left_points
                    total_left_points += left_points
        if COURSES_TYPES[0] in left_points_per_type.keys():
            if left_points_per_type[COURSES_TYPES[0]] >= (73.5 - MANDATORY_COURSES_POINTS):
                final_relevant_courses.extend(courses[COURSES_TYPES[0]][:MAX_AVAILABLE_COURSES-1])
                if len(courses[COURSES_TYPES[0]]) < MAX_AVAILABLE_COURSES:
                    total_points += len(courses[COURSES_TYPES[0]])
                    del courses[COURSES_TYPES[0]]
                else:
                    return final_relevant_courses

        # if the student finish A list but not a project
        if state.degree_requirements[COURSES_TYPES[6]][1] <= 0 and state.degree_requirements[COURSES_TYPES[8]][1] == 1:
            if courses.get(COURSES_TYPES[6]):
                for c_id in courses[COURSES_TYPES[6]]:
                    if Data.get_instance().courses_dict[c_id].get_is_project():
                        final_relevant_courses.append(c_id)
                        total_points += 1
                        del courses[COURSES_TYPES[6]]
                        break

        if state.degree_requirements[COURSES_TYPES[4]][1] <= 0 and state.degree_requirements[COURSES_TYPES[9]][1] == 1:
            total_left_points += 3
            left_points_per_type[COURSES_TYPES[4]] = 3
        ...
        for typ in courses:
            ...
            total_per_type = 0
            percentage_points = (left_points_per_type[typ] / total_left_points) * MAX_AVAILABLE_COURSES
            ...
            for i in range(len(courses[typ])):
                if total_per_type + 1 > percentage_points:
                    ...
                    total_points += 1
                    total_per_type += 1
            if total_points >= MAX_AVAILABLE_COURSES:
                break

        i = 0
        while total_points < MAX_AVAILABLE_COURSES and i < len(remaining_courses):
    ...
    def select_random_courses(self, courses):
        while True:
            if len(courses) < 7:
                return courses
            num_courses = random.randint(5, 7)  # Randomly select the number of courses
            selected_courses = random.sample(courses, num_courses)  # Randomly select the courses
            if 18 <= sum([Data.get_instance().courses_dict[course].get_credit_points() for course in selected_courses]) <= 23:  # Check if the sum of the selected courses is within the desired range
    ...
    def remove_unrelevant_courses(self, state: State, available_courses, semester: str):
        ...
        sorted_by_importance = sorted(available_courses, key=lambda x: Data.get_instance().courses_dict[x].get_importance(), reverse=True)
        for course_id in sorted_by_importance:
            ...
            if course.get_is_advanced():
                if not state.is_advanced_allowed():
                    continue
            # if len(relevant_courses) >= MAX_AVAILABLE_COURSES:
            #     break
        ...
        #make relevant courses to be a list of courses ids
        # courses = []
        # for typ in relevant_courses:
        #     courses.extend(relevant_courses[typ])
        # return self.select_random_courses(courses)


        if len(self.__seminars) > 0:
            if 'seminar' in Data.get_instance().courses_dict.keys():
                Data.get_instance().courses_dict['000004'].set_type(COURSES_TYPES[6])
                ...
                Data.get_instance().courses_dict['000004'].set_type(COURSES_TYPES[6])
                Data.get_instance().courses_dict['000004'].set_contains(relevant_seminars)
            if relevant_courses.get(COURSES_TYPES[6]):
                relevant_courses[COURSES_TYPES[6]].append('000004')
            else:
                relevant_courses[COURSES_TYPES[6]] = ['000004']

        # here the courses are already shorted by importance
        all_courses = []
        sum_courses = 0
        for typ in relevant_courses:
            all_courses.extend(relevant_courses[typ])
            sum_courses += len(relevant_courses[typ])
            if sum_courses > MAX_AVAILABLE_COURSES:
                break
        if sum_courses <= MAX_AVAILABLE_COURSES:
            return all_courses
    ...
    def has_irrelevant_courses(self, state: State, semester: [str]):
        ...
        for course_id in semester:
            if new_state.is_it_irrelevant_course(Data.get_instance().courses_dict[course_id].get_type(), course_id):
                ...
                if Data.get_instance().courses_dict[course_id].get_type() == COURSES_TYPES[4]:
    ...
    def check_tests_dates(self, semester: list[str]):
        ...
        study_days = timedelta(days=DAYS_BETWEEN_TESTS)
        dates = sorted(dates)
    ...
    def generate_semesters(self, state: State) -> set[str]:
        ...
        local_min_points = MIN_POINTS_PER_SEMESTER
        ...
        while optional_semesters == set():
            ...
            for sem in semesters_to_filter:
                ...
                if not self.is_linked_courses_legal(state, sem):
                    continue
                if not self.check_tests_dates(sem):
                    continue
                if not self.is_semester_can_be_scheduled(sem):
                    continue
    ...
    def is_it_a_promising_path(self, state: State, length: int):
        ...
        minimum_possible_length = math.ceil(remaining_points / MAX_POINTS_PER_SEMESTER) + length
        if minimum_possible_length > self.__shortest_path_length:
            self.__cut_counter += 1
            # print('________________________CUT____________________')
            # print('minimum possible len: ', minimum_possible_length)
            # print('known shortest len: ', self.__shortest_path_length)
    ...
    def add_optional_semesters_from_current_state(self, origin_node: (), state: State, season: str, length: int):
        ...
        if LENGTH_OPTIMIZATION:
            if not self.is_it_a_promising_path(new_state, length):
                return
        ...
        if self.__available_courses_list:
            ...
            if not self.options_coounter.get(length) or (self.options_coounter[length] < len(optional_semesters_set)):
                self.options_coounter[length] = len(optional_semesters_set)
    ...
    def create_graph(self) -> None:
        ...
        if available_courses:
            ...
            first_season = SEASON[0]  # start calculate from semester A --> the student will enter this detail?
            ...
            # print('Number of courses: ', len(self.__available_courses_list))
            self.options_coounter[1] = len(optional_semesters_set)
            if len(self.__available_courses_list) == 0:
                print('kk')
            # self.options_graph.add_nodes_from(optional_semesters_set)
...
#print number of nodes and edges
        ...
        # for i in self.options_coounter:
        #     print('length: ', i , 'num of options: ', self.options_coounter[i])
        # nx.write_gexf(self.options_graph, "test.gexf")
        # nx.write(self.options_graph, "test.gexf")

    def delete_instance(cls):
        cls.options_graph = None
        cls.__semesters = []
        cls.__available_courses_list = []
        cls.__seminars = []
        cls.linked_courses_dict = {}
        cls.cache_schedules = {}
        cls.__shortest_path_length = 15
        cls.__cut_counter = 0
        cls.options_coounter ={}
