...
class CoursesGraph(Singleton):
    ...
    @staticmethod
    def get_instance():
        if CoursesGraph._instance is None:
            raise Exception("Courses Graph instance does not exist")
        return CoursesGraph._instance
    ...
    def get_valid_courses(self, current_state: list[course_id]) -> (list[course_id], list[(first, second)]):
