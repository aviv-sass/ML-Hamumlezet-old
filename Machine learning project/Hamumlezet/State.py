...
from settings import COURSES_TYPES, COURSES_NUM_PER_SUBTYPES
...
class State:
    def __init__(self, completed_courses_id: list[str], requirements_credit_points: dict[COURSES_TYPES: float]):
    ...
    def is_remaining_points(self, total_points) -> bool:
        return self.get_remaining_points() <= total_points
    ...
    def get_completed_points(self):
        total_done_points = 0
        for typ in self.degree_requirements:
            done = self.degree_requirements[typ][0]
            total_done_points += done
        return total_done_points
    ...
    def get_total_points(self):
        total_points = 0
        for typ in self.degree_requirements:
            total_points += self.degree_requirements[typ][0]
            total_points += self.degree_requirements[typ][1]
        pass